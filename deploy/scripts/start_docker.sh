...
AWS_ACCOUNT_ID="739275446561"
ECR_REGISTRY="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com"
ECR_REPO="test-mlops/yt-chrome-plugin"
IMAGE_LOCAL_NAME="test-mlops/yt-chrome-plugin"
IMAGE_TAG="latest"
...
# Step 2: Stopping and removing any existing container
echo "Stopping and removing any existing container..."
docker rm -f ${CONTAINER_NAME} || true

# Step 3: Pulling the latest image from ECR
echo "Pulling latest image from ECR..."
docker pull ${IMAGE_NAME}

# Step 4: Running new container
echo "Running new container..."
docker run -d -p 80:5000 \
  -e DAGSHUB_PAT=7bed6b5be2021b1a4eaae221787bcb048ab2bcfd \
  --name ${CONTAINER_NAME} \
